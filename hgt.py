...
def data_export(file_name):
    file = '/root/nc/data/hgt/' + file_name
    print(file + "  已读取。")
    ...
    hgts = dataset.variables['hgt'][:].data # (time,level,lat,lon)
    ...
    for tendays in range(36):
        hgt_tendayly.append(sum(hgts[tendays*10:tendays*10+steps])/steps)
    
    hgt_tendayly = np.array(hgt_tendayly)
    print(file + "  旬数据已生成。")
    ...
    for months in range(12):
        hgt_monthly.append(sum(hgts[months*30:months*30+steps])/steps)
    
    hgt_monthly = np.array(hgt_monthly)
    print(file + "  月数据已生成。")
    
    #年
    steps = 365  # 时间步长
    hgt_yearly = []
    hgt_yearly.append(sum(hgts[0:365])/365)
    hgt_yearly = np.array(hgt_yearly)
    print(file + "  年数据已生成。")
    
    #旬
    tendayly_file = nc.Dataset('/root/nc/output/tendayly/hgt/'+ file_name[4:8] +'_hgt_tendayly.nc', 'w', format='NETCDF4')
    ...
    tendayly_file.createDimension('hgt_tendayly', len(hgt_tendayly))
    ...
    hgt_t = tendayly_file.createVariable('hgt_tendayly', np.float32, dimensions=('time', 'level', 'lat', 'lon'))
    ...
    hgt_t.description = "某旬某高度层某点位势高度，米"
    ...
    tendayly_file.variables['hgt_tendayly'][:] = hgt_tendayly
    ...
    #月
    monthly_file = nc.Dataset('/root/nc/output/monthly/hgt/'+ file_name[4:8] +'_hgt_monthly.nc', 'w', format='NETCDF4')
    ...
    monthly_file.createDimension('hgt_monthly', len(hgt_monthly))
    ...
    hgt_m = monthly_file.createVariable('hgt_monthly', np.float32, dimensions=('time', 'level', 'lat', 'lon'))
    ...
    time.units = 'hours since 1800-01-01 00:00:0.0'
    time.start_time = "1800-01-01 00:00:00"
    hgt_m.description = "某月某高度层某点位势高度，米"
    ...
    monthly_file.variables['hgt_monthly'][:] = hgt_monthly
    
    monthly_file.close()
    print(file_name + " 月数据已写入。")
    
    #年
    yearly_file = nc.Dataset('/root/nc/output/yearly/hgt/'+ file_name[4:8] +'_hgt_yearly.nc', 'w', format='NETCDF4')
    ...
    yearly_file.createDimension('hgt_yearly', len(hgt_yearly))
    ...
    hgt_y = yearly_file.createVariable('hgt_yearly', np.float32, dimensions=('time', 'level', 'lat', 'lon'))
    ...
    time.units = 'hours since 1800-01-01 00:00:0.0'
    time.start_time = "1800-01-01 00:00:00"
    hgt_y.description = "某年某高度层某点位势高度，米"
    ...
    yearly_file.variables['hgt_yearly'][:] = hgt_yearly
